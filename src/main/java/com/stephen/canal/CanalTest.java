...
public class CanalTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //canal.user =
        //canal.passwd =
        CanalConnector connector = CanalConnectors.newSingleConnector(new InetSocketAddress("192.168.28.129", 11111), "example", "", "");
        try {
            connector.connect();
            System.out.println("连接成功");
            //监听的表，格式为数据库.表名,数据库.表名
            // connector.subscribe(".*\\..*");
            // connector.subscribe("hello.tercher");
            connector.subscribe("hello.tercher,hello.users");
            connector.rollback();
